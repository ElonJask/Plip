...
- 验证：`swift test --disable-sandbox`。打包：`bash Scripts/make_app.sh`。只编译当前机器的架构，签名失败会中止。产物在 `artifacts/Plip-macos-<arch>.app` 和 `artifacts/Plip-v<version>-macos-<arch>.zip`。发布时 `macos-latest` 打 arm64，`macos-26-intel` 打 x86_64。
